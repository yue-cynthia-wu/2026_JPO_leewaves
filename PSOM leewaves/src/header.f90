...
#include "size.h"
  ...
  REAL(kind=rc_kind) :: Nbkgrnd                        !@ Nbkgnd: background buoyancy frequency
  ...
  INTEGER            :: iv_compute_kz,iv_compute_kh
  REAL(kind=rc_kind) :: A0x,A0y                        !@ variables used in mixing_horizontal (in m*m/s), in namelist
  REAL(kind=rc_kind) :: B0x,B0y                        !@ variables used in mixing_horizontal_biharmonic (in m^4/s)
  REAL(kind=rc_kind) :: KzMom,KzTr                     !@ variables used in mixing_vertical (in m*m/s), in namelist
  LOGICAL, PARAMETER :: bottom_linear_drag=.TRUE.      !@ only linear drag is currently supported
  REAL(kind=rc_kind) :: RR                             !@ bottom friction (in m/s), initialized in namelist
  REAL(kind=rc_kind) :: gFrc                           !@ non-dimensional number, gradient Fr
  REAL(kind=rc_kind) :: dz_avg                         !@ averaged vertical grid size, for parameterization of Kz
...
! REAL(kind=rc_kind) :: stressmax                                  !@ stressmax: defines the maximum stress, used in diffusion_wind.
  ...
  REAL(kind=rc_kind), dimension(      NI,    NJ,   0:NK  )     :: czf,wf !,Kz
  REAL(kind=rc_kind), dimension(    0:NI+1,0:NJ+1, 0:NK+1)     :: Kz,Kx,Ky                        ! Changed
