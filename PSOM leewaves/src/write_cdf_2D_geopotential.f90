subroutine write_cdf_2D_geopotential(zd,counter_2d,n)

  !     ------------------------------------------------------------------
USE header,ONLY : dx,dy,NI,NJ,NK,ntr,h,consump,Tr,s,T,rho,u,v,w,p,pv,vor,strain,freqN2,   &
 & xc,yc,zc,DL,LEN,Jac,nconsume,time_seconds,time_days,dirout,rc_kind
...
!      INCLUDE '/sw/lib/netcdf-gfortran/include/netcdf.inc'             
      ...
      REAL(kind=8) ::  xsurf(NI),ysurf(NJ),zsurf(NI,NJ),ssurf(NI,NJ),              &
         ...
         do i=1,NI 
            ...
            do k=2,NK 
              if (zc(i,j,k)*DL .ge. zd) then 
                ...
                pvsurf(i,j)     = sigup*pv(i,j,k)    + sigdn*pv(i,j,k-1)    
  ...
  102 l=0 
     ...
     do j=2,NJ-1
       do i=1,NI
         ...
         vorsurf(i,j)=0.5* ( (vsurf(ip1,j)-vsurf(im1,j))/dx-(usurf(i,j+1)-usurf(i,j-1))/dy )
       enddo
     enddo
