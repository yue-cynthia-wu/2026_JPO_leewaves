...
!    REAL(kind=rc_kind), dimension(      NI,  NJ,     0:NK  )     :: Jkfc
!    REAL(kind=rc_kind), dimension(      NI,  NJ,     0:NK,  2  ) :: gk !,gqk    
...
!       do 22 i=1,NI
!          do 32 j=0,NJ
!             Jkfc(i,j,k)= 0.5d0*(Jac(i,j,k)+ Jac(i,j,k+1))
!             gk(i,j,k,1)= 0.5d0*(g13(i,j) +g13(i,j+1))*Jkfc(i,j,k)
!             gk(i,j,k,2)= 0.5d0*(g23(i,j) +g23(i,j+1))*Jkfc(i,j,k)
! !             gqj(i,j,k,1)= qpr*gj(i,j,k,1)
! !             gqj(i,j,k,2)= qpr*gj(i,j,k,2)
!    32    continue
!    22 continue
...
!       ! ------------------------------------------------------------
!       ! interpolation of Jac, g11, g12 and gi on the face z. !!! ADDED
!       do k=1,1
!          do i=1,NI
!             do j=0,NJ
!                Jkfc(i,j,k)= 0.5d0*(Jac(i,j,k)+ Jac(i,j,k+1))
